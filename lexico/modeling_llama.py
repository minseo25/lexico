...
from lexico.generation_utils import GenerationConfig, GenerationMixinLexico
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        key_dictionaries: List[torch.Tensor] = None,
        value_dictionaries: List[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            compressed_key_states, key_states, compressed_value_states, value_states = past_key_value.update(
                key_states,
                value_states,
                key_dictionaries[self.layer_idx], 
                value_dictionaries[self.layer_idx], 
                self.layer_idx, 
                cache_kwargs,
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            # sin and cos are specific to RoPE models; cache_position needed for the static cache
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
            compressed_key_states, key_states, value_states = past_key_value.update(
                key_states,
                value_states,
                key_dictionaries[self.layer_idx], 
                value_dictionaries[self.layer_idx], 
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            position_ids=position_ids,
            past_key_value=past_key_value,
            key_dictionaries=key_dictionaries,
...
class LlamaForCausalLMLexico(LlamaPreTrainedModel, GenerationMixinLexico):
    ...
    def __init__(self, config):
        ...
        self.load_dictionaries(config, getattr(config, "dictionary_device", "cuda"))

        # Initialize weights and apply final processing
        self.post_init()
    
    def load_dictionaries(self, config, device):
        ...
        if dictionary_path is None:
            raise FileNotFoundError(f"No dictionary found for {config_key}")
        
        D = torch.load(dictionary_path).to(torch.float16)

        self.key_dictionaries = []
        self.value_dictionaries = []

        for i in range(D.shape[0]):
            if i % 2 == 0:
                self.key_dictionaries.append(D[i].to(device))
            else:
                self.value_dictionaries.append(D[i].to(device)) 
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        outputs = self.model(
            ...
            key_dictionaries=self.key_dictionaries,
            value_dictionaries=self.value_dictionaries,
